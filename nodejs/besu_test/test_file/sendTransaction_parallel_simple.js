...
// 로그 파일 경로 설정
const logFilePath = path.join(__dirname, 'transaction_parallel_simple_logs.txt');
...
async function sendTx(tx, index) {
    try {
        ...
        const logMessage = `[${index}] 트랜잭션 전송 성공: ${receipt.transactionHash}`;
        console.log(logMessage);
        writeLog(logMessage);
        return receipt;
    } catch (error) {
        const logMessage = `[${index}] 트랜잭션 전송 오류: ${error.message}`;
...
async function sendMultipleTxs(numTxs) {
    ...
    // Nonce 값 가져오기 및 변환
    const nonceBigInt = await web3.eth.getTransactionCount(account); // BigInt로 반환될 가능성
    const nonce = Number(nonceBigInt); // 숫자로 변환
