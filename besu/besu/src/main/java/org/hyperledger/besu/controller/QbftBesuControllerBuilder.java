...
import org.hyperledger.besu.consensus.common.bft.PoolFulfilled;
...
public class QbftBesuControllerBuilder extends BftBesuControllerBuilder {
  ...
  private BftFinalState finalState;
  private MutableBlockchain blockchain;


  /**
   * Default Constructor.
   * 생성자를 통해 이벤트 버스에 해당 클래스를 등록 
   * */
  public QbftBesuControllerBuilder() {
            EventBus.register(event -> {
            if (event instanceof PoolFullEvent) {
                handlePoolFullEvent();
            }
        });
  }
  ...
  protected MiningCoordinator createMiningCoordinator(
    ...
    finalState =
        new BftFinalState(
            ...
            new PoolFulfilled(bftEventQueue),
            blockCreatorFactory,
            clock);
  ...
  private void handlePoolFullEvent() {
    long nextBlockHeight = blockchain.getChainHeadHeader().getNumber() + 1;
    final ConsensusRoundIdentifier roundIdentifier = new ConsensusRoundIdentifier(nextBlockHeight, 0); 
    // 트랜잭션 풀 가득 참 이벤트 처리 로직
    bftEventQueue.add(new PoolCapacityFulfilled(roundIdentifier));
  }
}
