...
public abstract class BaseBftController implements BftEventHandler {
  ...
  public void handleBlockTimerExpiry(final BlockTimerExpiry blockTimerExpiry) {
    // 만료된 타이머가 속한 합의 라운드 정보 가져오기
    final ConsensusRoundIdentifier roundIdentifier = blockTimerExpiry.getRoundIdentifier();
    // 만료된 타이머의 블록 높이와 현재 블록 높이를 비교
    // true : 동일하다면, 타이머 만료 로직 실행
    // false : 다르다면, 현재 블록 높이와 이벤트가 발생한 블록 높이를 log로 기록
  ...
  @Override
  public void poolCapacityFulfilled(final PoolCapacityFulfilled poolCapacityFulfilled) {
    // 만료된 타이머가 속한 합의 라운드 정보 가져오기
    final ConsensusRoundIdentifier roundIdentifier = poolCapacityFulfilled.getRoundIdentifier();
    // 만료된 타이머의 블록 높이와 현재 블록 높이를 비교
    // true : 동일하다면, 타이머 만료 로직 실행
    // false : 다르다면, 현재 블록 높이와 이벤트가 발생한 블록 높이를 log로 기록
    if (isMsgForCurrentHeight(roundIdentifier)) {
      getCurrentHeightManager().poolCapacityFulfilled(roundIdentifier); // 블록 타이머 초기화 단계 필요
    } else {
      LOG.trace(
          "Transaction Pool Fulfilled event discarded as it is not for current block height chainHeight={} eventHeight={}",
          getCurrentHeightManager().getChainHeight(),
          roundIdentifier.getSequenceNumber());
    }
