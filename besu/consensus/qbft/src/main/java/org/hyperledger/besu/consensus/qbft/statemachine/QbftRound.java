...
public class QbftRound {
  ...
  public void startRoundWith(final RoundChangeArtifacts roundChangeArtifacts, final long headerTimestamp) {
    // PreparedCertificate 조회
    // PreparedCertificate : 이전 라운드에서 준비된 블록을 나타냄
    ...
    // 블록을 네트워크에 전송
    // 라운드 상태를 업데이트하여 새 라운드를 정상적으로 시작
  ...
  private void importBlockToChain() {
    ...
    if (!result.isImported()) {
      ...
      if (AbstractPendingTransactionsSorter.spaceUsed > 11000) {
