...
public class TimestampMoreRecentThanParent implements DetachedBlockHeaderValidationRule {
  ...
  private boolean validateHeaderSufficientlyAheadOfParent(final long timestamp, final long parentTimestamp) {
    final long secondsSinceParent = timestamp - parentTimestamp;
    if (secondsSinceParent < minimumSecondsSinceParent) {
      LOG.info(
          "오류 아님(최소 시간 간격 이내에 제출 작업 진행되는 지 확인용) Invalid block header: timestamp {} is only {} seconds newer than parent timestamp {}. Minimum {} seconds",
          timestamp,
          secondsSinceParent,
          parentTimestamp,
